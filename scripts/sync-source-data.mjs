...
function main() {
  ...
  const tsPath = join(outDir, "source-data.ts");
  const legacyJsonPath = join(outDir, "source-data.json");
  const tsBody = JSON.stringify(payload, null, 2);
  writeFileSync(
    tsPath,
    `// Auto-generated by scripts/sync-source-data.mjs — do not edit.\nexport const sourceDataJson = ${tsBody} as const;\n`,
  );
  if (existsSync(legacyJsonPath)) rmSync(legacyJsonPath);
  console.log(
    `Synced source-data.ts — ${website.rifles.length} rifles, ${website.optics.length} optics, ${website.stockColors.length} colors`,
  );
}
